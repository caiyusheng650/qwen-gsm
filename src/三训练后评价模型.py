...
def main():
    ...
    with open("evaluation_results_finetuned.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print("\nResults saved to evaluation_results_finetuned.json")
    
    # 保存详细结果到CSV文件
    df = pd.DataFrame(results_data)
    df.to_csv("三评价结果训练后.csv", index=False, encoding='utf-8-sig')
    print("Detailed results saved to 三评价结果训练后.csv")
